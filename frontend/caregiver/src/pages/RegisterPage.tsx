...
export default function RegisterPage() {
  ...
  async function requestOtp(event: React.FormEvent) {
    ...
    setError(null)
    try {
      const result = await authApi.register('caregiver', phone)
  ...
  async function verify(event: React.FormEvent) {
    ...
    setError(null)
    try {
      const session = await authApi.verifyOtpAndRegister('caregiver', { phone, code, password })
      setSession(session)
      navigate('/onboard')
    } catch (err) {
      setError((err as ApiError).message || th.common.error_generic)
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center gap-6 p-6">
      <header className="text-center">
        <h1 className="text-2xl font-bold text-amber-700">{th.auth.register}</h1>
        <p className="mt-1 text-gray-500">{th.caregiver.portalName}</p>
      </header>
